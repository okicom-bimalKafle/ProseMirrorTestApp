...
/**
 * --------------------- NOTE ---------------------
 * The keyboard immediately shows up but the user list is not scrollable. 
 */
...
onMounted(() => {
    ...
    console.log('UserAgent Name: ', navigator.userAgent);
    if (
        ua.indexOf('iphone') > -1 ||
...
const onMentionNameClick= (
    ...
    setIsOpen(false);
    commentFocusOn();
    if (actionType == "click" && isTouchDevice.value) {
        console.log('False Trigger: ', 'actionType', actionType, 'siTouchDevice', isTouchDevice.value)
        return;
    }
    if (actionType == "touch" && !isTouchDevice.value) {
        console.log('False Trigger: ', 'actionType', actionType, 'siTouchDevice', isTouchDevice.value)
        return;
    }
    
    console.log('Correct Trigger:', 'actionType', actionType, 'isTouchDevice', isTouchDevice.value);
...
const handleScroll = async () => {
    ...
    console.log('scrollTop', containers?.scrollTop, 'clientHeight', containers?.clientHeight, 'scrollHeight', containers?.scrollHeight)
    if (
        containers &&
